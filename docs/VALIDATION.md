...
|---|---|
| GCC/G++ 13.3.0, C11/C++17, Debug, `-Wall -Wextra -Wpedantic -Werror` | All 28 programs, 20 C++ test executables and one C test executable built successfully |
| Normal build, MPI and OpenGL enabled | 24/24 CTest groups passed |
| AddressSanitizer + UndefinedBehaviorSanitizer build, MPI and OpenGL enabled | 24/24 CTest groups passed; no reported sanitizer defects |
...
- File errors, malformed numeric input, long palindrome strings, C-string capacity boundaries, command-line product overflow/sign/zero cases, and two OpenGL framebuffer pixel checks.
